...
namespace v2rayN.Desktop.Views
{
    public partial class MainWindow : ReactiveWindow<MainWindowViewModel>
    {
        ...
        private async Task<bool> UpdateViewHandler(EViewAction action, object? obj)
        {
            switch (action)
            {
                ...
                case EViewAction.AddServerViaClipboard:
                    ////ViewModel?.AddServerViaRestApiAsync();
        ...
        //{
        //    Task<string> task1 = Task<string>.Factory.StartNew(() => ViewModel?.GetProxyByApiSSL());
        //    var tasks = new List<Task>();
        //    tasks.Add(ViewModel?.GetProxyByApiSSL());
        //    tasks.Add(ViewModel?.AddServerViaClipboardAsync(ApiData));

        //    await Task.WhenAll(tasks).ConfigureAwait(false);
        //}
        private async void AddProxyTask()
        { 
            while (true)
            {
                ////var ApiData = await ViewModel?.GetProxyByApiSSL();
                ////ViewModel?.AddServerViaClipboardAsync(ApiData);
                ViewModel?.AddServerViaRestApiAsync();
